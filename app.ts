...
export class WallPolygonMeshBuilder extends BABYLON.PolygonMeshBuilder {
  constructor(name, polygonCorners, scene) {
    super(name, polygonCorners, scene);
  }
  wallBuilder = function (w0, w1) {
    var positions = [];
    var iuvs = [];
    var euvs = [];
    var icolors = [];
...
export default class App {
  ...
  public createScene(engine: BABYLON.Engine): BABYLON.Scene {
    ...
    this.buildFromPlan(data, 0.3, 3.9, opt, scene);
    return scene;
  }
  ...
  public createData(): Wall[] {
    ...
    for (let b = 0; b < baseData.length / 2; b++) {
      corners.push(
        new BABYLON.Vector3(baseData[2 * b], baseData[2 * b + 1], 0)
      );
    }
  ...
  public buildFromPlan(
    ...
    if (nbWalls === 2) {
      ...
      for (var w = 0; w < nbWalls - 1; w++) {
        ...
        let lineNormal = new BABYLON.Vector3(
          line.z,
          0,
          -1 * line.x
        ).normalize();
    ...
    for (var w = 0; w < nbWalls; w++) {
      innerTopCorners.push(
        new BABYLON.Vector3(
          innerBaseCorners[w].x,
          height,
          innerBaseCorners[w].z
        )
      );
      outerTopCorners.push(
        new BABYLON.Vector3(
          outerBaseCorners[w].x,
          height,
          outerBaseCorners[w].z
        )
      );
    }
    ...
    for (var w = 0; w < nbWalls - 1; w++) {
      ...
      exteriorWallLength = outerBaseCorners[w + 1]
      ...
      for (var d = 0; d < doors; d++) {
        polygonCorners.push(
          new BABYLON.Vector2(walls[w].doorSpaces[d].left, 0)
        );
        polygonCorners.push(
          new BABYLON.Vector2(
            walls[w].doorSpaces[d].left,
            walls[w].doorSpaces[d].opening.height
          )
        );
        polygonCorners.push(
          new BABYLON.Vector2(
            walls[w].doorSpaces[d].left + walls[w].doorSpaces[d].opening.width,
            walls[w].doorSpaces[d].opening.height
          )
        );
        polygonCorners.push(
      ...
      polygonTriangulation = new WallPolygonMeshBuilder(
        '',
        polygonCorners,
        scene
      );
      ...
      for (var ws = 0; ws < windows; ws++) {
        ...
        holeData.push(
          new BABYLON.Vector2(
            walls[w].windowSpaces[ws].left,
            height -
              walls[w].windowSpaces[ws].top -
              walls[w].windowSpaces[ws].opening.height
          )
        );
        holeData.push(
          new BABYLON.Vector2(
            walls[w].windowSpaces[ws].left +
              walls[w].windowSpaces[ws].opening.width,
            height -
              walls[w].windowSpaces[ws].top -
              walls[w].windowSpaces[ws].opening.height
          )
      ...
      wallData.positions.push(
        outerBaseCorners[w].x,
        outerBaseCorners[w].y,
        outerBaseCorners[w].z
      );
      wallData.positions = wallData.positions.concat(doorData);
      wallData.positions.push(
        outerBaseCorners[w + 1].x,
        outerBaseCorners[w + 1].y,
        outerBaseCorners[(w + 1) % nbWalls].z
      );
      wallData.positions.push(
        outerTopCorners[w + 1].x,
        outerTopCorners[w + 1].y,
      ...
      if (doorsRemaining > 0) {
        ...
        positions.push(
          ...
          innerDoorCorners[w][doorNb][0].z
        ); //tr
        positions.push(
          outerDoorCorners[w][doorNb][0].x,
          outerDoorCorners[w][doorNb][0].y,
          outerDoorCorners[w][doorNb][0].z
        ); //br

        uvs.push(
          exteriorUV.x,
          exteriorUV.y + ((exteriorUV.w - exteriorUV.y) * ply) / maxH
        ); //top Left
        ...
        uvs.push(
          ...
          exteriorUV.y +
            ((exteriorUV.w - exteriorUV.y) *
              walls[w].doorSpaces[doorNb].opening.height) /
              maxH
        ); //top right
        uvs.push(exteriorUV.x, exteriorUV.y); //base Left
        uvs.push(
        ...
        uvs.push(
          exteriorUV.x +
            ((exteriorUV.z - exteriorUV.x) *
              walls[w].doorSpaces[doorNb].opening.width) /
              maxL,
          exteriorUV.y
        ); //base right
        uvs.push(
      ...
      while (doorsRemaining > 0) {
        ...
        positions.push(
          ...
          innerDoorCorners[w][doorNb - 1][3].z
        ); //bl
        positions.push(
          innerDoorCorners[w][doorNb][0].x,
          innerDoorCorners[w][doorNb][0].y,
        ...
        positions.push(
          ...
          outerDoorCorners[w][doorNb - 1][3].z
        ); //tl
        positions.push(
          outerDoorCorners[w][doorNb][0].x,
          outerDoorCorners[w][doorNb][0].y,
            ...
            ((exteriorUV.z - exteriorUV.x) *
              (walls[w].doorSpaces[doorNb].left -
                (walls[w].doorSpaces[doorNb - 1].left +
                  walls[w].doorSpaces[doorNb - 1].opening.width))) /
              maxL /
              maxL,
          exteriorUV.y
        ); //base right
      ...
      if (doors > 0) {
        ...
        positions.push(
          innerBaseCorners[w + 1].x,
          innerBaseCorners[w + 1].y,
          innerBaseCorners[w + 1].z
        ); //br
        positions.push(
          outerDoorCorners[w][doorNb][3].x,
          outerDoorCorners[w][doorNb][3].y,
      ...
      for (let ww = 0; ww < innerWindowCorners[w].length; ww++) {
        ...
        uvs.push(
          ...
          exteriorUV.y +
            ((exteriorUV.w - exteriorUV.y) *
              walls[w].windowSpaces[ww].opening.height) /
              maxH
        ); //top right
        uvs.push(
          exteriorUV.x + ((exteriorUV.z - exteriorUV.x) * ply) / maxL,
        ...
        uvs.push(
          exteriorUV.x +
            ((exteriorUV.z - exteriorUV.x) *
              walls[w].windowSpaces[ww].opening.width) /
              maxL,
          exteriorUV.y + ((exteriorUV.w - exteriorUV.y) * ply) / maxH
        ); //top right
        uvs.push(exteriorUV.x, exteriorUV.y); //base Left
      ...
      positions.push(
        innerTopCorners[w + 1].x,
        innerTopCorners[w + 1].y,
        innerTopCorners[w + 1].z
      ); //tr
      positions.push(
        outerTopCorners[w].x,
        outerTopCorners[w].y,
      ...
      if (interior) {
        ...
        positions.push(
          innerBaseCorners[0].x,
          innerBaseCorners[0].y,
          innerBaseCorners[0].z
        );
        positions.push(
          outerBaseCorners[0].x,
          outerBaseCorners[0].y,
          outerBaseCorners[0].z
        );
        positions.push(
          outerTopCorners[0].x,
          outerTopCorners[0].y,
        ...
        positions.push(
          innerBaseCorners[nbWalls - 1].x,
          innerBaseCorners[nbWalls - 1].y,
          innerBaseCorners[nbWalls - 1].z
        );
        positions.push(
          outerBaseCorners[nbWalls - 1].x,
          outerBaseCorners[nbWalls - 1].y,
          outerBaseCorners[nbWalls - 1].z
        );
        positions.push(
          outerTopCorners[nbWalls - 1].x,
          outerTopCorners[nbWalls - 1].y,
          outerTopCorners[nbWalls - 1].z
        );
        positions.push(
          innerTopCorners[nbWalls - 1].x,
          innerTopCorners[nbWalls - 1].y,
          innerTopCorners[nbWalls - 1].z
        );
